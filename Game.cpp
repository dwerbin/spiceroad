...
Game::Game() : turn_id(-1),
               players(), objective_deck(), claimables(), gold_coin_count(0), silver_coin_count(0), merchant_deck(), buyables()
{
    ...
    merchant_deck.resize(std::size(productions) + std::size(trades) + std::size(upgrades));
...
{
    return (get_current_player().get_objectives().size() >= get_objective_count_to_win());
}
...
void Game::execute() {
    ...
    do {
        ...
        if (get_round_id() > 150) {
            ...
            std::cout << player.get_name() << "'s turn:" << std::setw(41) << std::right << "R" << get_round_id() << "T" << get_turn_id() << std::endl;
...
//-------------------
...
#if 1
        std::cout << "T" << std::setfill('0') << std::setw(3) << get_turn_id() << "R" << std::setw(2) << get_round_id() << std::setfill(' ') << std::endl;
        std::cout << "          C:" << player.get_caravan() << std::endl;
        if (const BasicStrategy* basic = dynamic_cast<const BasicStrategy*>(player.get_strategy())) {
            std::cout << "       -> F:" << *basic->get_focus() << std::endl; 
        }
        std::cout << "          " << *action;
        if (ClaimAction* claim = dynamic_cast<ClaimAction*>(action.get())) {
            const Objective* claimable = get_claimable(claim->get_claimable_index());
            std::cout << "                                         " << *claimable;
        ...
        if (action->valid(*this, player)) {
            ...
            throw std::runtime_error("The game cannot execute an invalid action !");
        }
