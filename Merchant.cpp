...
Production::Production(std::initializer_list<int> spice_map) : Merchant(Type::PRODUCTION),
                                                               spice_map(spice_map)
{
}
...
Trade::Trade(std::initializer_list<int> spice_map) : Merchant(Type::TRADE),
                                                     spice_map(spice_map)
{
}
...
Upgrade::Upgrade(unsigned int upgrade_times) : Merchant(Type::UPGRADE),
...
Buyable::Buyable(const Merchant *merchant) : merchant(merchant),
                                             deposit()
{
}
...
const SpiceMap &Buyable::get_deposit() const
{
    return deposit;
}

const Merchant *Buyable::get_merchant() const
{
    return merchant;
}
...
void Buyable::add_deposit(const SpiceMap &new_deposit)
{
    ...
    {
        throw std::runtime_error("Cannot add a negative spice map to a buyable deposit");
    }
    deposit.apply(new_deposit);
}

void Buyable::add_deposit(Spice spice)
{
    deposit.add(spice);
}
