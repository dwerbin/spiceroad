...
void Player::play(const Upgrade& card, const SpiceMap& upgrade_map) {
    auto ite = check_merchant_card_in_hand(card);
    if (((int)card.get_upgrade_times()) >= upgrade_map.get_value() && upgrade_map.sum() == 0) {
    ...
    if (caravan.get_quantity() > MAX_SPICE_IN_CARAVAN)
    {
        if (strategy) {
            ...
            if (!drop_map.is_negative()) {
                throw std::runtime_error("Cannot drop spice : non negative drop map");
            } else if (!caravan.can_apply(drop_map)) {
                throw std::runtime_error("Cannot drop spice : cannot apply the drop map to the caravan");
            } else {
        ...
        if (caravan.get_quantity() > MAX_SPICE_IN_CARAVAN)
        {
            throw std::runtime_error("Too many spice in caravan after a drop of spice");
        }
    }
}

std::unordered_set<const Merchant*>::iterator Player::check_merchant_card_in_hand(const Merchant& card) {
    std::unordered_set<const Merchant*>::iterator ite = hand.find(&card);
    if (ite == hand.end()) {
        throw std::runtime_error("Cannot play a merchant card wich is not in the hand of the player"); 
    }
    return ite;
}

void Player::discard(std::unordered_set<const Merchant*>::iterator hand_iterator) {
    discard_pile.insert(*hand_iterator);
    hand.erase(hand_iterator);
}

unsigned int Player::get_score(bool with_spice_bonuses) const {
    unsigned int score = std::accumulate(objectives.begin(), objectives.end(), 0UL, [](unsigned int s, const Objective* o) { return s + o->get_value(); });
...
void Player::add_gold_coin() {
    gold_coins++;
}

void Player::add_silver_coin() {
    silver_coins++;
}
