...
{
    memcpy(quantities, that_quantities.begin(), std::min(SPICE_TYPE_COUNT, that_quantities.size()) * sizeof(int));
}
...
void SpiceMap::apply(const SpiceMap &that, unsigned int times, bool check_positive)
{
    ...
    for (size_t spice = 0; spice < SPICE_TYPE_COUNT; ++spice)
    {
        ...
        {
            throw std::runtime_error("Cannot apply a spice map to another : the destination space map should stay positive");
        }
...
int SpiceMap::sum() const {
    return std::accumulate(quantities, quantities+SPICE_TYPE_COUNT, 0, std::plus<int>()); 
}
...
{
    return stream << "{" << 
        std::setw(2) << spice_map.quantities[0] << ", " << 
