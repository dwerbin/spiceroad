...
Action* BasicStrategy::play_turn(const Game& game, const Player& player) {
    ...
    for(unsigned int i=0; i<buyables.size(); ++i) {
        if (caravan_quantity >= i) {
            ...
            SpiceMap drop_map = std::move(drop_spice(caravan, caravan_quantity-i));
            SpiceMap next_caravan(caravan);
            next_caravan.apply(drop_map);
            next_caravan.apply(buyable.get_deposit());
            SpiceMap drop_map_max = std::move(drop_spice(next_caravan, Player::MAX_SPICE_IN_CARAVAN));
            ...
            // try to apply buyable.get_merchant() to the next_caravan, if it makes earn even more than anything in hand : take it
            unsigned int next_distance2 = next_distance;
            Action* dummy_action = try_to_apply_all_combination(buyable.get_merchant(), next_caravan, next_distance2, nullptr);
    ...
    for(const Merchant* merchant : hand) {
        Action* new_action = try_to_apply_all_combination(merchant, player.get_caravan(), action_distance, &action_value);
    ...
    for(const Merchant* merchant : discard) {
        Action* dummy_action = try_to_apply_all_combination(merchant, player.get_caravan(), action_distance2, nullptr);
        if (dummy_action != nullptr) {
            delete action2;
    ...
    if (player.get_discard().empty()) {
        std::cout << "uhoh... Local minimal... SleepAction call on an empty discard pile..." << std::endl;
    }

    return new SleepAction();
}

Action* BasicStrategy::try_to_apply_all_combination(const Merchant* merchant, const SpiceMap& caravan, unsigned int& distance, unsigned int* action_value) const {
    Action* action = nullptr;
    ...
    switch (merchant->get_type()) {
        case Merchant::Type::PRODUCTION:
        {
            ...
            next_caravan.apply(drop_spice(next_caravan, Player::MAX_SPICE_IN_CARAVAN));
            unsigned int next_distance = focus->distance(next_caravan);
            unsigned int next_value = next_caravan.get_value();
            if ( (next_distance <  action_distance) || 
                ((next_distance == action_distance) && (action_value != nullptr) &&  (next_value > *action_value))) {
                delete action;
        ...
        case Merchant::Type::UPGRADE:
        {
            ...
            std::vector<SpiceMap> all_possible_combination_of_upgrade(compute_all_upgrade_combination(upgrade_times, caravan));
...
static void add_to_possible_upgrade_map(unsigned int upgrade_times, size_t first_spice, const SpiceMap& upgrade_map, const SpiceMap& caravan, std::vector<SpiceMap>& result) {
    for (size_t spice = first_spice; spice < (SPICE_TYPE_COUNT - 1); ++spice)
    {
        ...
        if (upgrade_times == 1) {
            ...
            add_to_possible_upgrade_map(upgrade_times-1, spice, current_upgrade_map, caravan, result);
        }
    }
}

std::vector<SpiceMap> BasicStrategy::compute_all_upgrade_combination(unsigned int max_upgrade_times, const SpiceMap& caravan) {
    std::vector<SpiceMap> result;
    for(unsigned int upgrade_times=1; upgrade_times<=max_upgrade_times; ++upgrade_times) {
        SpiceMap empty_map;
        add_to_possible_upgrade_map(upgrade_times, 0, empty_map, caravan, result);
    }
    return result;
}

const Objective* BasicStrategy::select_objective(const Game& game, const Player& player) {
    ...
    for(const Objective* o : claimables) {
        if(focus == nullptr) {
            ...
            if ((d < d_focus) || ((d == d_focus) && (focus->get_value() < o->get_value()))) {
                focus = o;
                d_focus = d;
            }
        }
...
SpiceMap BasicStrategy::drop_spice(const Game& game, const Player& player) {
    SpiceMap drop_map = std::move(drop_spice(player.get_caravan(), Player::MAX_SPICE_IN_CARAVAN));
    return drop_map;
}

void BasicStrategy::build_all_possible_drop_map(std::set<SpiceMap>& result, const SpiceMap& caravan0, const SpiceMap& caravan, unsigned int quantity_to_drop) const {
    if (quantity_to_drop > 0) {
        for (size_t spice = 0; spice < SPICE_TYPE_COUNT; ++spice)
        {
            ...
            if (caravan.get_quantity(s) > 0) {
                ...
                build_all_possible_drop_map(result, caravan0, next_caravan, quantity_to_drop - 1);
            }
        }
    } else {
...
SpiceMap BasicStrategy::drop_spice(const SpiceMap& caravan, unsigned int max_spice) const {
    #if 1
    SpiceMap drop_map;
    // drop as many spice as necessary starting with the yellow, then the red, etc...
    int remaining = caravan.get_quantity();
    size_t spice = 0;
    while (spice < SPICE_TYPE_COUNT && remaining > (int) max_spice)
    {
        Spice s = (Spice)spice;
        unsigned int q_to_remove_from_s = std::min(remaining - (int) max_spice, caravan.get_quantity(s));
    ...
    if (quantity_to_drop > 0) {
        ...
        build_all_possible_drop_map(drop_maps, caravan, caravan, quantity_to_drop);
...
SpiceMap InteractiveStrategy::drop_spice(const Game& game, const Player& player) {
    SpiceMap result;
    //TODO 
    return result;
}
